...
def define_discriminator(n_inputs=15):
	model = Sequential()
...
def generate_real_samples(n):
    X = pd.read_csv('../data/train_input/CASE_01.csv')
    X = X.drop(['obs_time'], axis=1)
    X = np.array(X)
    y = ones((n, 1))
    return X, y
...
def train_discriminator(model, n_epochs=1000, n_batch=128):
	...
	for i in range(n_epochs):
		...
		X_real, y_real = generate_real_samples(data_size)
		# update model
		model.train_on_batch(X_real, y_real)
		# generate fake examples
		X_fake, y_fake = generate_fake_samples(data_size)
		# update model
...
def summarize_performance(epoch, generator, discriminator, latent_dim, n=672):
...
def train(g_model, d_model, gan_model, latent_dim, n_epochs=10000, n_batch=128, n_eval=50):
	# determine half the size of one batch, for updating the discriminator
	half_batch = int(n_batch / 2)
	# manually enumerate epochs
	for i in range(n_epochs):
		# 실제 데이터를 생성하고
		x_real, y_real = generate_real_samples(672)

		# 가짜 데이터는 Generator의 Predict로 만들어 낸다
		x_fake, y_fake = generate_fake_samples(g_model, latent_dim, 672)
		# 분별자(discriminator)를 생성한 데이터로 학습하고
		d_model.train_on_batch(x_real, y_real)
		d_model.train_on_batch(x_fake, y_fake)
		# prepare points in latent space as input for the generator
		x_gan = generate_latent_points(latent_dim, 672)
		# create inverted labels for the fake samples
		y_gan = ones((672, 1))
